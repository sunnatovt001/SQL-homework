...
--EASY--
	...
	select count (CustomerID) as NumberRows from Customers
	...
	select sum (ProductID) as SalesAmount from Sales
	where ProductID=7
	...
	select count (DepartmentName) as EachDepartment from Employees
...
---Medium---
	...
	select ProductID, sum (SaleAmount) as TotalSales, avg (SaleAmount) as AverageSales from Sales
	group by ProductID 
	
	--Write a query to count the number of employees from the Department HR.
	select count(DepartmentName) as DepartmentHR from Employees
	...
	select DepartmentName, count (FirstName) as CountEmployee,avg(Salary) as AverageSalary from Employees 
	group by DepartmentName

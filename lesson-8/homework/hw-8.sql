...
--EASY--
	...
	select Category,count(ProductName) as TotalProduct from Products
	group by Category
	...
	select Country,count (FirstName) as EachCountry from #Customers
	group by Country
...
--MEDIUM--
	...
	select Year(OrderDate) as OrderYear,avg(TotalAmount) as AvgAmount from Orders
	...
	pivot (sum(Population) for year in ([2012],[2013])) as pvt

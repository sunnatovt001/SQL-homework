...
INNER JOIN Customers AS C ON O.CustomerID = C.CustomerID;
	...
	select p.ProductName,o.CustomerID from #Products as p
	...
	select p.PaymentMethod,o.OrderID,o.CustomerID from Payments as p
	join Orders as o on p.OrderID=o.OrderID

	--Using Orders, Products table Show orders where product price is more than 100.
	select p.ProductName,p.Category,p.Price,o.TotalAmount from Orders as o
...
--MEDIUM--
	...
	cross join Departments  as d where e.DepartmentID!=d.DepartmentID
	----------------------------------------------------------------------------------------------------------------------
	--Using Orders, Products table Show orders where ordered quantity is greater than stock quantity.
	select 
	o.OrderID,
	p.ProductName,
	o.Quantity,								-----oxirgacham bitmagan
	p.StockQuantity 
	from Orders as o
	join #Products as p on o.ProductID=p.ProductID
...
WHERE 
	...
	select c.CustomerID,c.FirstName,s.ProductID,s.SaleAmount from #Customers as c
...
----------------------------------------------------------------------------------------------------------------------
	...
	where c.CategoryName like 'Elect%' or c.CategoryName like 'Furni%'

	--Using Sales, Customers table Show all sales from customers who are from 'USA'.
	select s.CustomerID,s.ProductID,s.SaleAmount,s.SaleDate,s.SaleID,c.FirstName,c.City,c.Country from Sales as s
	...
	select c.Country,c.City,o.TotalAmount,o.Quantity,o.OrderDate from Orders as o
...
--HARD--
	...
	select p.PaymentID, p.Amount, o.Quantity, pr.Price, (o.Quantity * pr.Price) from Payments as p
	join Orders as o on p.OrderID=o.OrderID
	...
	select a.Name,a.Salary,b.Name,b.Salary from Employees as a
	join Employees as b on a.EmployeeID=b.ManagerID
	where a.Salary<=b.Salary

	--Using Orders, Payments, Customers table List customers who have made an order, but no payment has been recorded for it.
	select o.CustomerID,c.FirstName from Orders as o
	join #Customers as c on o.CustomerID=c.CustomerID

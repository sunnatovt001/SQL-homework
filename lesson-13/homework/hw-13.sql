...
INSERT INTO Orderss (CustomerID, OrderID, DeliveryState, Amount) VALUES
...
--EASY--
	...
	select LEN(FIRST_NAME) as NameLenght,FIRST_NAME from Employees
...
--MEDIUM--
	...
	select 
		...
		SUBSTRING(FullName,CHARINDEX(' ',FullName,1)+1,CHARINDEX(' ',FullName,CHARINDEX(' ',FullName,1)+1) - CHARINDEX(' ',FullName,1)),
		SUBSTRING(FullName,CHARINDEX(' ',FullName,CHARINDEX(' ',FullName,1)+1) +1,len(FullName)-CHARINDEX(' ',FullName,CHARINDEX(' ',FullName,1)+1))
	...
	select distinct * from Orderss as a
	join Orderss as b on a.CustomerID=b.CustomerID
	where a.DeliveryState = 'CA' and b.DeliveryState = 'TX'

	--Write an SQL statement that can group concatenate the following values.(DMLTable)
	select*from DMLTable

	SELECT SequenceNumber, 
       STRING_AGG(String, ', ') AS ValuesList
FROM DMLTable
GROUP BY SequenceNumber;
	...
	from Studentss
	where FullName like '%a%' and FullName like '%a%' and FullName like '%a%'
...
GROUP BY JOB_ID;
	...
	select JobDescription,max(MissionCount),min(MissionCount) from Personal
...
SELECT
    STRING_AGG(CASE WHEN ch ~ '[A-Z]' THEN ch ELSE '' END, '') AS Uppercase,
    STRING_AGG(CASE WHEN ch ~ '[a-z]' THEN ch ELSE '' END, '') AS Lowercase,
    STRING_AGG(CASE WHEN ch ~ '[0-9]' THEN ch ELSE '' END, '') AS Numbers,
    STRING_AGG(CASE WHEN ch ~ '[^A-Za-z0-9]' THEN ch ELSE '' END, '') AS Others
FROM Split;
...
SELECT 
    StudentID,
    Value,
    SUM(Value) OVER (ORDER BY StudentID ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS CumulativeSum
FROM Studentss

----------------------------------------------
SELECT 
    Equation,
    eval(Equation) AS Result   -- PostgreSQLda "pg_eval" extension kerak
FROM Equations;
